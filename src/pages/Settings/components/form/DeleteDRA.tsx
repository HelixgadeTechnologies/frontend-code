...
export const DeleteDRA = observer(({
    close,
    userId,
    store,
    from
    ...
    const handleRemoveDRA = async () => {
        try {
            if (from === "pending") {
                const response = await store.declinePendingDra(userId)
                if (response) {
                    toast.success("DRA successfully removed");
                    close();
                }
            } else {
                const response = await store.removeDra(userId)
                if (response) {
                    toast.success("DRA successfully removed");
                    close();
                }
