...
export const PendingAdminTable = observer(() => {
    ...
    const filteredData = useMemo(() => {
        const pendingAdmins = [...settingStore.allPendingAdmin.values()];
        if (!searchTerm) return pendingAdmins;

        return pendingAdmins.filter((user) => {
            const fullName = `${user.firstName} ${user.lastName}`.toLowerCase();
            const email = user?.email?.toLowerCase();
            const search = searchTerm.toLowerCase();
            return fullName.includes(search) || email?.includes(search);
        });
    }, [settingStore.allPendingAdmin, searchTerm]);
        ...
        <div className="mt-10 bg-white p-4 rounded-2xl border border-gray-8 ">
            <section className="mb-4 flex items-center justify-between gap-x-3">
                <div className="flex-1 max-w-sm">
                    <input
                        type="text"
                        placeholder="Search pending admin..."
