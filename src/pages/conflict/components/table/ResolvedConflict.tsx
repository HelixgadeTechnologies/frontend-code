...
export const ResolvedConflict = observer(({ conflictStore }: { conflictStore: IConflictStore }) => {
    ...
    const handleView = useCallback((conflict: IConflictView | null = null) => {
        conflictStore.resolvedConflict = conflict
    }, []);
        ...
        <div className="mt-10 bg-white p-4 rounded-2xl border border-gray-8 ">
            <>
                {conflictStore.dashboardData == null ? (
                    ...
                    <Table
                        columns={columns}
                        data={conflictStore?.dashboardData?.RESOLVED_CONFLICTS?.map((conflict: IConflictView, i: number) => ({
