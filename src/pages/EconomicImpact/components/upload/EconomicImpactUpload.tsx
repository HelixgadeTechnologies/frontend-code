...
import { IEconomicImpactUploadValidationResponse } from "../../types/interface";
...
const EconomicImpactUpload = observer(() => {
    ...
    const onFile = useCallback(async (file: File | null) => {
        if (!file) return;
        economicImpactStore.isValidate = true;
        economicImpactStore.uploadErrorCount = 0;
        await economicImpactStore.validateEconomicImpactExcel(file);
        ...
        reader.onload = (e) => {
            ...
            try {
                ...
                const sheetName = wb.SheetNames.find((n) => n.toLowerCase() === "economic impact")
                    || wb.SheetNames.find((n) => n.toLowerCase() === "database")
                    || wb.SheetNames[0];
                const ws = wb.Sheets[sheetName];
                const json: Array<Record<string, any>> = XLSX.utils.sheet_to_json(ws, { defval: "" });
                setRows(json);
                const head = json.length > 0 ? Object.keys(json[0]) : economicImpactKeys;
                setHeaders(head);
                setSelected({});
            } catch (err) {
    ...
    const saveValidatedData = async () => {
        try {
            const res = await economicImpactStore.saveValidatedEconomicImpactData(rows);
            if (res) {
                setRows([]);
                toast.success("Economic impact data saved successfully.");
        ...
        <div className="py-6 px-4 max-w-7xl mx-auto">
            ...
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-4 bg-gray-100 p-1 rounded-lg">
                    <button
                        onClick={() => economicImpactStore.activeUploadTab = 0}
                        className={`px-4 py-2 rounded-md transition-all ${economicImpactStore.activeUploadTab === 0 ? 'bg-white shadow-sm font-semibold text-primary-600' : 'text-gray-600 hover:text-gray-800'}`}
                    >
                        Upload
                    </button>
                    <button
                        onClick={() => economicImpactStore.activeUploadTab = 1}
                        className={`relative px-4 py-2 rounded-md transition-all ${economicImpactStore.activeUploadTab === 1 ? 'bg-white shadow-sm font-semibold text-primary-600' : 'text-gray-600 hover:text-gray-800'}`}
                    >
                        Console
                        {economicImpactStore.uploadErrorCount > 0 && (
                            <span className="ml-2 inline-flex items-center justify-center absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold rounded-full w-5 h-5 shadow-sm">
                                {economicImpactStore.uploadErrorCount}
                            </span>
                        )}
                    </button>
                </div>
                <div>
                    <Button
                        type="button"
                        disabled={economicImpactStore.uploadErrorCount > 0 || rows.length === 0 || uploading}
                        loading={economicImpactStore.isSubmitting}
            ...
            <div className="mb-6 flex flex-wrap items-center gap-4 text-sm font-medium">
                ...
                <button
                    ...
                    onClick={() => {
                        ...
                        setFileName('');
                        setSelected({});
                        economicImpactStore.uploadValidationResult = {} as IEconomicImpactUploadValidationResponse;
                        economicImpactStore.uploadErrorCount = 0;
                ...
                <button
                    ...
                    onClick={() => {
                        ...
                        const ws = XLSX.utils.json_to_sheet(sampleData, { header: economicImpactKeys });
                        const wb = XLSX.utils.book_new();
                        XLSX.utils.book_append_sheet(wb, ws, 'Economic Impact');
                        XLSX.writeFile(wb, 'economic-impact-template.xlsx');
                    }}
            ...
            {economicImpactStore.activeUploadTab === 0 && (
                <div className="space-y-4">
                    ...
                    >
                        <div className="flex flex-col items-center justify-center text-center">
                            ...
                            <h3 className="text-lg font-semibold text-gray-900 mb-1">Upload Economic Impact Sheet</h3>
                            <p className="text-sm text-gray-500 mb-6 max-w-sm">
                                Click or drag your Excel template (.xlsx) with a sheet named <strong>Economic Impact</strong> to this area
                            </p>
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                        {uploading ? (
                            ...
                            <div className="flex items-center gap-3 text-gray-500">
                                ...
                                <span className="text-sm">Select a file to start validation. Required columns: trustName, businessGrowth, incomeIncrease, livelihoodImprove, accessAmenities.</span>
                            </div>
                        )}
            ...
            {economicImpactStore.activeUploadTab === 1 && (
                    ...
                    <div className="p-6 max-h-[400px] overflow-auto">
                        {economicImpactStore.uploadErrorCount === 0 ? (
                            ...
                            <div className="space-y-3">
                                {economicImpactStore.uploadValidationResult?.validationSummary?.map((m: any, i: number) => (
                                ...
                                {economicImpactStore.uploadResponse?.failed?.map((m: any, i: number) => (
...
export default EconomicImpactUpload;
